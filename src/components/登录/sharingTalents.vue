...
<div>
  ...
  <bg>
    <template v-slot:topTab>
      ...
      <i id="after2" :style="tabSelect==1?'left:188px':'left:0px'"></i>
    </template>
      ...
      <div class="form_reg">
        ...
        <div v-else key="a2">
          ...
          <label class="reg-label">
            ...
            <a>平台服务协议</a>
            <font>、</font>
            <a href="javascript:void(0);">用户平台入驻协议</a>
            <font>、</font>
...
export default {
  ...
  methods: {
    ...
    regsubmit: function() {
      this.$router.push({ path: "/login_complete" });
      sessionStorage.setItem('thisMyUserName',this.emailForm.email)
      sessionStorage.setItem('thisMyUserpassword',this.emailForm.email_pwd)
    },
    //邮箱验证
    youxiang_yz() {
      setTimeout(() => {
    ...
    regsubmit() {
      postReg({
        ...
        if (res.code === 200) {
            ...
            sessionStorage.setItem("username", this.emailForm.email);
            let shenfenData={
    ...
    regsubmit2() {
      postReg({
        ...
        if (res.code === 200) {
          sessionStorage.setItem("user", res.data);
          sessionStorage.setItem("username", this.phoneForm.phone);
          let shenfenData={
              username:sessionStorage.getItem("username"),
              shenType:3
          }
