        ...
        <div class="publishing">
            <div class="publishing_nav">
                <ul class="publishing_nav_title">
                    <li class="publishing_selected" >
                        <font class="publishing_selected_fontBloder">我的待评价</font>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    methods: {
        ...
        routerToDetails(personId){
            this.$router.push({path:"my_project_details",query:{"personId":personId,listType:this.listType,personalEvaluate:"1"}})
        },
        //调用
    ...
    mounted() {
        ...
        //查询了所有需求 默认全部
        let user=JSON.parse(sessionStorage.getItem("user"));
        getPersonNeeds({listType:"我的需求",indivId:user.roleId,personStatus:"待评价"}).then(res => {
            if (res.data.code === 200) {
                this.arrSUXQ=res.data.data.records.reverse();
                this.fsNumber=res.data.data.total;
